...
- [x] stable declaration `resume-en-operator-multi-planet-2026-08-14`;
- [x] exact-target current-state qualifier and focused synthetic tests;
...
- **Current state:** representation split selected; the exact-target lifetime experiment now has an integrated qualification → observer → transcript-to-record → evidence-bundle path validated synthetically, but no exact-target lifetime record has been produced yet. Reuse-safe sidecar identity/lifetime and the separate lossless Manual-transition invalidation boundary remain open.
- **Execution:** CLOUD
- **Priority:** Critical
...
The Stage 1 exact-target census remains a reviewed negative result: [`docs/experiments/A1-sidecar-identity-static-census-result.md`](./docs/experiments/A1-sidecar-identity-static-census-result.md) records **Outcome C — no adequate static seam** (`static`, clean blind-RE provenance). Its 38 references to `DS:0x43660`, 109 to `DS:0x43664`, 36 decoded `0x7b` contexts, and initializer-shaped `+0x5a = 0` leaf do not establish a construction/reset event, array base/count, stable slot index, generation boundary, or complete writer set.

Since that census, the repository has built and reviewed the bounded runtime evidence chain rather than gaining new target evidence. [`docs/experiments/A1-runtime-scenario-qualification-contract.md`](./docs/experiments/A1-runtime-scenario-qualification-contract.md) defines the predeclared selection-control/replacement scenarios and v2 witness ranges; [`docs/experiments/A1-exact-target-lifetime-observer-contract.md`](./docs/experiments/A1-exact-target-lifetime-observer-contract.md) defines the exact-target observer boundary; [`docs/experiments/A1-runtime-observer-orchestration.md`](./docs/experiments/A1-runtime-observer-orchestration.md) wires bounded orchestration and evidence validation; and [`docs/experiments/A1-observer-record-schema-integration-gap.md`](./docs/experiments/A1-observer-record-schema-integration-gap.md) records the closed digest/range schema-integration defect. The integrated producer/adapter path is exercised by the repository-safe [`docs/experiments/A1-synthetic-lifetime-record.json`](./docs/experiments/A1-synthetic-lifetime-record.json). These are **tooling/synthetic** results only and do not establish reuse-safe identity on the canonical target.

The earlier producer gap tracked by issue #108 is closed, so it no longer blocks the experiment. **Exactly one next bounded A1 evidence action is current:** implement the thin exact-target observer behind the existing orchestration interface and run the already-defined `selection-control`, `new-game-reset`, and `save-load-replacement` plan against canonical `ANTAG.EXE`. Reuse the established runtime mapping and v2 witness verifier; preserve the predeclared bounds and negative/no-safe-seam outcome if no valid epoch/reuse detector appears. Do not promote planet name to immutable identity, infer a slot from stride, or weaken the oracle to obtain a positive result.

That lifetime run will answer the identity half only. The separate lossless Manual-transition invalidation requirement remains open and must be established independently before A1 can complete.
...
- **Status:** Investigation first
- **Current state:** Stage 1 exact-target capacity inventory and all four existing A2 lead producers have now been compared on canonical `ANTAG_EN.EXE` (`static`, clean blind-RE provenance) in [`docs/experiments/A2-four-producer-lead-set-comparison.md`](./docs/experiments/A2-four-producer-lead-set-comparison.md). The four current-head outputs contain 10 / 340 / 1,983 / 2,021 hit rows. After normalizing to `(candidate range, candidate-relative offset)`, the decoded model's 40 unique leads are contained in both broader raw sets, the 10 linear-only leads are contained in both raw sets, and the all-four intersection is empty because the 10-hit producer uses only `linear-va` while the decoded producer's hits are `target-object-relative`. Candidate ranking agrees only by majority: three producers rank `0x96c10` first and decoded memory ranks `0x988dc` first. No existing mapped range is established reusable, no mechanism is selected, and no mapped bytes are promoted on set intersection alone. The bounded runtime-capacity observer and Stage 2 LE-growth transformer remain prepared; neither is replaced by a new producer.
- **Execution:** CLOUD
- **Priority:** Critical
...
The current repository has stronger follow-up machinery prepared but not promoted beyond its evidence class. [`docs/experiments/A2-decoded-memory-reference-triage.md`](./docs/experiments/A2-decoded-memory-reference-triage.md) records the decoded absolute-memory probe and its exact-target output as target-specific leads, not semantic references. [`docs/experiments/A2-four-producer-lead-set-comparison.md`](./docs/experiments/A2-four-producer-lead-set-comparison.md) records the bounded cross-producer result: lead containment is present among the compatible target-relative models, but the ranking split and address-model boundary do not establish semantic consumers or reusable capacity. [`docs/experiments/A2-runtime-capacity-observation.md`](./docs/experiments/A2-runtime-capacity-observation.md) provides a bounded read-only exact-target observer, validated synthetically, that can distinguish initial nonzero materialization or runtime mutation from a bounded no-write observation while the target progresses. [`docs/experiments/A2-stage2-le-growth-transformer.md`](./docs/experiments/A2-stage2-le-growth-transformer.md) is likewise target-neutral synthetic preparation: it deliberately rejects the canonical target and does not establish loader acceptance, runtime execution of appended pages, or mechanism B.

Mechanisms A, B, and C therefore remain unselected. The four-producer comparison closes issue #119 without adding instrumentation. **Exactly one next bounded A2 evidence action remains current:** run the already-prepared exact-target runtime capacity observation against canonical `ANTAG.EXE` with the verified retail fixture:

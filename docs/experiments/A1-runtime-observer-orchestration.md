...
`scripts/run_a1_lifetime_observer.py` builds the scenario manifest from independently supplied qualification bytes before starting the observer, gives the observer only that manifest plus a fresh record-output path, bounds observer execution with a timeout, and immediately validates the detached record with the existing A1 evidence bundle.

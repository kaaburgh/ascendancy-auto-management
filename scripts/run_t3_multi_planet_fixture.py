...
def harness_source_paths() -> dict[str, Path]:
    """Return the complete source closure used by the T3 runtime oracle."""
    return {
        "run_t3_multi_planet_fixture.py": Path(__file__).resolve(),
        "run_re4_runtime_state.py": Path(re4.__file__).resolve(),
        "run_re5_runtime_turn_path.py": Path(re5.__file__).resolve(),
        "run_re5_override_witness.py": Path(override_witness.__file__).resolve(),
        "le_image.py": Path(override_witness.le_image.__file__).resolve(),
    }


def resolve_harness_snapshot_dir(snapshot_dir: Path, game_dir: Path) -> Path:
    """Require durable evidence snapshots under docs/experiments, away from target inputs."""
    resolved = snapshot_dir.expanduser().resolve()
    experiments = (ROOT / "docs" / "experiments").resolve()
    try:
        resolved.relative_to(experiments)
    except ValueError as exc:
        raise T3Error("harness-snapshot-dir must be under docs/experiments") from exc
    ...
    try:
        ...
        raise T3Error("harness-snapshot-dir must not be inside the source game tree")
    if resolved.exists() and not resolved.is_dir():
        raise T3Error("harness-snapshot-dir exists but is not a directory")
    return resolved


def harness_provenance(snapshot_dir: Path) -> dict[str, Any]:
    """Preserve exact executed source bytes and return content-addressed artifact fields."""
    ...
    for name, source in sources.items():
        ...
        if destination.exists() and destination.read_bytes() != data:
            raise T3Error(
                f"harness snapshot {destination} already exists with different bytes; "
        ...
        try:
            ...
            raise T3Error("harness snapshot escaped repository evidence tree") from exc
        hashes[name] = digest
        snapshots[name] = relative.as_posix()
    return {
        "runner_source_sha256": hashes["run_t3_multi_planet_fixture.py"],
        "harness_dependencies": {
            name: digest
            for name, digest in hashes.items()
            if name != "run_t3_multi_planet_fixture.py"
        },
        "harness_source_snapshots": snapshots,
        "runner_identity_note": (
            "Commit ids are intentionally not persisted because review maintenance uses rebase/rebase-merge. "
            "The runner and complete dependency closure are identified by SHA-256 and exact source snapshots."
        ),
    }
...
def run(args: argparse.Namespace) -> dict[str, Any]:
    ...
    provenance = harness_provenance(args.harness_snapshot_dir)
    return {
...
def parser() -> argparse.ArgumentParser:
    ...
    result.add_argument(
        ...
        type=Path,
        help=(
            "Durable source snapshot directory under docs/experiments. Defaults to "
            "<artifact-stem>-harness next to an artifact that itself is under docs/experiments."
        ),
    )
    return result
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        requested_snapshot_dir = args.harness_snapshot_dir
        if requested_snapshot_dir is None:
            requested_snapshot_dir = artifact.with_name(artifact.stem + "-harness")
        args.harness_snapshot_dir = resolve_harness_snapshot_dir(
            requested_snapshot_dir, args.game_dir
        )

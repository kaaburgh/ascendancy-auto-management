...
int main(void){SDL_SetVideoMode(640,480,32,0);for(int i=0;i<100;i++){SDL_Event ev;if(SDL_PollEvent(&ev)&&ev.type==2&&ev.key.keysym.sym==32){usleep(160000);return 0;}usleep(5000);}return 3;}
'''
...
class ProbeSmokeTests(unittest.TestCase):
    def test_probe_compiles_in_isolation(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as name:
            ...
            subprocess.run([gcc, "-shared", "-fPIC", "-O2", "-Wall", "-Wextra", "-Werror", "-pthread", "-o", str(output), str(PROBE), "-ldl"], check=True)
            self.assertTrue(output.is_file())

    def test_safe_fixture_gets_key_and_framebuffer_capture(self) -> None:
        gcc = shutil.which("gcc")
        self.assertIsNotNone(gcc, "gcc is required by the CF3 cloud harness")
        with tempfile.TemporaryDirectory() as name:
            ...
            subprocess.run([gcc, "-shared", "-fPIC", "-O2", "-Wall", "-Wextra", "-Werror", "-pthread", "-o", str(probe), str(PROBE), "-ldl"], check=True)
            captures = temp / "captures"; captures.mkdir()
            env = os.environ.copy()
            env.update({"LD_PRELOAD": str(probe), "CF3_CAPTURE_DIR": str(captures), "CF3_KEY_EVENTS": "40:space", "CF3_CAPTURES_MS": "80"})
            ...
            frame = captures / "frame-000080ms.ppm"
